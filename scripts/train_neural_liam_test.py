...
def fit_model(x_train, y_train):
    ...
    max_vocab = 10000
    model = Sequential()
    model.add(layers.Embedding(max_vocab, output_dim=32))  # We have 25000 words in the vocabulary,
    model.add(layers.Bidirectional(tf.keras.layers.LSTM(64,  return_sequences=True))),
    model.add(layers.Bidirectional(tf.keras.layers.LSTM(16))),                                                 # and each word is represnted by a vector of size 128
    ...
    model.fit(x_train, y_train, epochs=10, batch_size=16, validation_split=0.3, callbacks=[es])
